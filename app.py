...
# Processing Data & Date Range Filter Setup
if not df_cash.empty:
    ...
    # 4. Sidebar - Date Filter Option
    st.sidebar.markdown("---")
    st.sidebar.header("📅 Date Filter (ရက်စွဲအလိုက် ရွေးရန်)")
    min_date = df_cash["Date_dt"].min().date()
    max_date = df_cash["Date_dt"].max().date()
    
    start_date = st.sidebar.date_input("Start Date (စတင်သည့်ရက်)", min_date)
    end_date = st.sidebar.date_input("End Date (ပြီးဆုံးသည့်ရက်)", max_date)
    
    # Filter Dataframe according to Date Range
    mask = (df_cash["Date_dt"].dt.date >= start_date) & (df_cash["Date_dt"].dt.date <= end_date)
    filtered_df = df_cash.loc[mask].reset_index(drop=True)
    ...
    if filtered_df.empty:
        st.warning("⚠️ ရွေးချယ်ထားသော Date Range အတွင်း မည်သည့် စာရင်းမျှ မရှိပါ။")
    else:
        st.info(f"📆 လက်ရှိပြသနေသော စာရင်းရက်စွဲ: **{start_date}** မှ **{end_date}** အထိ")

        # Metrics Overview
        total_cash_val = filtered_df[filtered_df["Payment_Type"] == "Cash"]["Amount"].sum()
        total_chq_val = filtered_df[filtered_df["Payment_Type"] == "Cheque"]["Amount"].sum()
        ...
        # Excel Export Function (Filtered)
        def convert_df_to_excel(df, s_date, e_date):
            ...
            grand_total_border = Border(top=Side(style='thin', color='000000'), bottom=Side(style='double', color='000000'))
            ...
            ws_detail.cell(row=2, column=1, value=f"Date Range: {s_date} to {e_date}").font = sub_title_font
            ...
            ws_summary.cell(row=2, column=1, value=f"Date Range: {s_date} to {e_date}").font = sub_title_font
        ...
        # PDF Generator Function (Filtered with Date Range)
        class PDFReport(FPDF):
            def header(self):
                self.set_font("Arial", 'B', 15)
                self.set_text_color(31, 73, 125)
                self.cell(0, 8, "Daily Collection & Weekly Settlement Report", ln=True, align='C')
                self.set_font("Arial", 'I', 8)
                self.set_text_color(100, 100, 100)
                self.cell(0, 5, "Generated by Enterprise ERP Collection System", ln=True, align='C')
                self.ln(3)
        ...
        def generate_pdf_report(df, s_date, e_date):
            pdf = PDFReport(orientation='P', unit='mm', format='A4')
            pdf.add_page()
            
            # Sub-header Date Range info
            pdf.set_font("Arial", 'B', 9)
            pdf.set_text_color(80, 80, 80)
            pdf.cell(0, 5, f"Filtered Range: {s_date} to {e_date}", ln=True, align='L')
            pdf.ln(2)

            # 1. Daily Collection Table
            pdf.set_font("Arial", 'B', 11)
            pdf.set_text_color(0, 0, 0)
            pdf.cell(0, 7, "1. Daily Collection Detail Logs", 0, 1, 'L')
            
            pdf.set_font("Arial", 'B', 9)
            pdf.set_fill_color(31, 73, 125)
            pdf.set_text_color(255, 255, 255)
            
            pdf.cell(25, 7, "Date", 1, 0, 'C', fill=True)
            pdf.cell(35, 7, "Collector", 1, 0, 'C', fill=True)
            pdf.cell(45, 7, "Customer", 1, 0, 'C', fill=True)
            pdf.cell(20, 7, "Type", 1, 0, 'C', fill=True)
            pdf.cell(30, 7, "Cheque No", 1, 0, 'C', fill=True)
            pdf.cell(35, 7, "Amount (MMK)", 1, 1, 'C', fill=True)
            ...
            pdf.set_font("Arial", 'B', 11)
            pdf.cell(0, 7, "2. Weekly Settlement Summary", 0, 1, 'L')
            ...
            pdf.cell(15, 7, "Year", 1, 0, 'C', fill=True)
            pdf.cell(20, 7, "Week No.", 1, 0, 'C', fill=True)
            pdf.cell(40, 7, "Collector", 1, 0, 'C', fill=True)
            pdf.cell(50, 7, "Customer", 1, 0, 'C', fill=True)
            pdf.cell(25, 7, "Type", 1, 0, 'C', fill=True)
            pdf.cell(40, 7, "Total Amount", 1, 1, 'C', fill=True)
            ...
            # 3. Overall Totals Box
            pdf.ln(5)
            pdf.set_font("Arial", 'B', 10)
            pdf.cell(0, 7, "Collection Overall Summary Totals:", 0, 1, 'L')
            ...
            pdf.cell(110, 6.5, "Grand Total Collected:", 1, 0, 'L', fill=True)
        ...
        excel_data = convert_df_to_excel(filtered_df, start_date, end_date)
        col_ex1.download_button(
            label="📊 Download Excel Report (Filtered Date Range)",
            data=excel_data,
            file_name=f"Collection_Report_{start_date}_to_{end_date}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

        try:
            pdf_bytes = generate_pdf_report(filtered_df, start_date, end_date)
            col_ex2.download_button(
                label="📄 Download PDF Report (Filtered Date Range)",
                data=pdf_bytes,
                file_name=f"Collection_Report_{start_date}_to_{end_date}.pdf",
                mime="application/pdf"
            )
        except Exception as e:
            col_ex2.error(f"⚠️ PDF ထုတ်ယူရာတွင် အမှားဖြစ်ပေါ်ပါသည်: {e}")

        st.markdown("---")

        # 5. Filtered Weekly Summary Table
        st.subheader("📅 Weekly Summary Report (သီတင်းပတ်အလိုက် စာရင်းချုပ်)")
        weekly_summary = filtered_df.groupby(["Year", "Week", "Collector", "Customer", "Payment_Type"])["Amount"].sum().reset_index()
        weekly_summary.columns = ["Year", "Week No.", "Collector Name", "Customer Name", "Payment Type", "Total Amount (MMK)"]
        st.dataframe(weekly_summary, use_container_width=True)

        # 6. Interactive Editable Filtered Daily Logs Table
        st.subheader("📝 Daily Transaction Detail Logs (Editable)")
        st.info("💡 ဇယားထဲတွင် ကလစ်နှိပ်၍ အချက်အလက်များ တိုက်ရိုက်ပြင်ဆင်နိုင်ပါသည်")
        
        editable_df = filtered_df[["Date", "Collector", "Customer", "Payment_Type", "Cheque_No", "Amount"]].copy()
        ...
        if st.button("💾 Save Table Edits"):
            # Update original dataframe with edited slice
            df_cash.update(edited_df)
            df_cash.to_csv(cash_file, index=False)
